---
title: "A propagation workflow with taxonomic pooling, asr (phytools), and inheritance (compact version)"
output:
    html_document:
        toc: true
---
...
## Map annotations (genus, species, strain) to NCBI taxonomic tree
...
## Perform first round of tax/inh1
...
no_annotated_tips <- matrix(
    data = rep(rep(1/ncol(m1), ncol(m1)), length(no_annotated_tips_chr)),
...
## Extract new taxa for second round of tax/inh2
...
add_taxa1 <- phys_data_ready |> 
    filter(!NCBI_ID %in% unique(output$NCBI_ID))
add_taxa2 <- new_taxa_for_ncbi_tree |> 
    filter(!NCBI_ID %in% unique(output$NCBI_ID))
