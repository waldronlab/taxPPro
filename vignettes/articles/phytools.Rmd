...
```{r}
Attribute_group_var <- unique(phys[[1]]$Attribute_group)
Attribute_group_var
```

```{r}
Attribute_type_var <- unique(phys[[1]]$Attribute_type)
Attribute_type_var
```
...
set_with_ids <- phys_data |> 
    ...
    mutate( # original frequency values need to be replaced. I wonder if this is a good idea.
        Frequency = case_when(
            Score == 1 ~ 'always',
            Score >= 0.9 & Score < 1 ~ 'usually',
            Score >= 0.5 & Score < 0.9 ~ 'sometimes',
...
ncbi_tree$Do(function(node) {
    if (!node$isRoot)
        node$taxid <- sub('^\\w__', '', node$name)
})
ncbi_tree$d__2$taxid
```
        ...
        if (conds) {
            res_tbl <- purrr::discard(attribute_tbls, is.null) |>
                ...
                mutate(
                    ...
                    Rank = case_when(
                        grepl('^g__', node$name) ~ 'genus',
                        grepl('^s__', node$name) ~ 'species',
                        grepl('t__', node$name) ~ 'strain'
                    )
                ) |> 
                distinct()
