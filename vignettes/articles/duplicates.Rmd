...
These are not necessarily conflicts, just taxa annotated more than once in the
same spreadsheet.
...
nrows_duplicates |> 
    ggplot(aes(reorder(dataset, n_rows), n_rows)) + 
    geom_col() + 
    geom_label(aes(label = n_rows)) +
    labs(
        title = 'Number of rows with duplicates across spreadsheets',
        x = 'dataset'
    ) +
    theme(
...
> Sitll need to know what is going on with habitat and antimicrobila resistance
Which have lots of duplicates. This needs manual inpsection per
spreadsheet/dataset
