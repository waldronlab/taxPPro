---
title: "Uncertainty in tips for binary attributes"
---
...
Some of the attributes in bugphyzz only have 'TRUE' annotations.
Some other attributes have both 'TRUE' and 'FALSE' annotations.

Tips that are uncertain could be treated as FALSE annotations or 
uncertain annotations with prior probabilities (0.5 TRUE and 0.5 FALSE). A
drawback of the later is that it could cause all annotations to have
the same value:
...
```{r}
set.seed(12343) ## with this seed all values are TRUE that is what I left it here
...
pos1 <- sample(1:nrow(m75), round(nrow(m75) * 0.75), replace = FALSE)
pos0 <- 1:nrow(m75) 1:nrow(m75)
