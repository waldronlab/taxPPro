...
```{r}
my_data_lists <- map(my_data, ~ {
    split(.x, .x$NCBI_ID)
})
my_data_lists <- my_data_lists[physNames]
length(my_data_lists)
```

Just taxonomic pooling

```{r}
just_taxpool <- vector('list', length(my_data_lists))
for (i in seq_along(just_taxpool)) { currentList <- my_data_lists[[i]]
    attributeType <- unique(currentList[[1]]$Attribute_type)
    attributeType <- attributeType[!is.na(attributeType)]
    
    attributeGroup <- unique(currentList[[1]]$Attribute_group)
    attributeGroup <- attributeType[!is.na(attributeGroup)]
    
    ncbi_tree$Do(function(node) {
        if (node$name %in% names(currentList)) {
            node$attribute_tbl <- currentList[[node$name]]
        }
    })
    
    ncbi_tree$Do(
        fun = function(node) {
            taxPool(node, grp = attributeGroup, typ = attributeType)
        }, traversal = 'post-order'
    )
    attrs <- ncbi_tree$Get(
        attribute = 'attribute_tbl', 
        filterFun = function(node) !is.null(node$attribute_tbl), 
        simplify = FALSE
    )
    just_taxpool[[i]] <- bind_rows(attrs)
}
```
...
Taxonomic pooling and inheritance
